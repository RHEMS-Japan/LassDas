...
func designReviewPrompt(input designReviewPromptInput) (string, error) {
	...
	head := []string{
		...
		"- 依頼者に質問することはできません。疑問は設計・依頼・実際の記録に照らして確かめ、必要な根拠の欠落や矛盾として検算できるものだけを findings にしてください。",
		"",
		"## 見る観点",
		input.lens,
		"",
	...
	if len(input.previous) > 0 {
		...
		middle = append(middle,
			"- 各指摘の前提と修正要求を、今回の設計・依頼・実際の記録に照らして検算してください。妥当で未解消のものだけを findings に含めて revise にしてください。前提や修正要求が誤っていれば、その要求どおりに直していないことを差し戻す理由にしないでください。",
	...
	tail = append(tail,
		...
		"- その時点の材料で裏付けられる指摘を評決として提出してください。未確認の懸念を不具合として作らず、必要な根拠が欠けている場合は、どの必須の主張が何の記録で支えられていないかを示してください。読み尽くすことより評決の提出を優先してください。",
		"",
