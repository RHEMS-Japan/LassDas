...
**「失敗しても止まらない」には、名前の付いた例外がある。** 次の条件では `internal_failed` で終端し、最終コメントを出す。いずれも本体の側では直せないもので、実行環境か納品先の設定を人が変えるまで、何度やり直しても同じところへ来る。
...
| 受付の判断を記録したファイルが、2 度続けて読めない | 同上 | 1 度目は本体が自分で受付をやり直すので、何もしなくてよい。2 度続けて読めないのはボリュームか受付そのものの故障なので、実行環境を見る |

**受付の記録が 1 度読めなくなっただけでは終わらない。** 読めない記録を消し、その巡のカードを片付けて、**受付をもう一度実行する。** やり直したことは前提として【実装方針】コメントに出る。2 度目も読めなければ上の表に入る — 無制限にやり直すのは、この設計が持たないことにした無限再試行そのものだからである。

**AI の答えの形は、もう終端の理由に入らない。** 以前は受付の読み取り役の答えを 3 回読めないと `internal_failed` で終わっていた。いまは本体が自分で読み、チケットの本文をそのまま実装役へ渡して進む（[AI の答えは、形が違っても読む](#ai-の答えは形が違っても読む)）。上の表の 3 つはどれも**保存先か設定の故障**で、答えの形は含まれない。
