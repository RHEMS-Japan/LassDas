...
func decodeTurnAnswer(encoded []byte, phase string) (turnAnswer, error) {
	...
	if err := decodeModelJSON(encoded, &answer, "probe", "read", "report", "design"); err != nil {
...
- A claim that something does not exist — no runbook says this, nothing documents that, there is no such setting — needs a record that searched for it, not a record that listed names. Cite the search. Unknown is not an alternative to checking a claim the requested change relies on: use relevant catalogue probes and read the recorded output while budgets allow. If it remains unknown, state what you tried and what prevented an answer (failed measurement, unavailable capability, truncated output or exhausted budget); distinguish not checked from not obtainable. This also applies when a revise round says to drop the claim or mark it unknown. Never invent evidence or exceed a budget.
- A value called normal, typical or a baseline describes more than one observation. Repeat a relevant measurement under stated conditions before making that claim, and report the observed variation; otherwise describe it only as a single observation with its time and target. Do not turn one sample into a general expectation.
- Before planning to write a factual value, search for the same claim elsewhere in the repository and read the matching passages. Keep statements about the same target, conditions and time consistent; distinguish historical observations instead of overwriting them to force agreement. If a necessary correction is outside the allowed change, name the conflict and limit the claim; do not expand the file scope.
- Copied identifiers and reproduction commands must match their evidence, including the full resource name and the target selection needed to repeat the observation. If a context, environment or namespace distinguishes otherwise ambiguous targets, make that selection explicit rather than relying on the operator's current default.
