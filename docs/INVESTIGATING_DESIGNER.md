...
- **会話の予算**: モデルに見せる抜粋と窓 (`read` で読んだ分) の合計は 1 会話 **256 KiB** まで。超えたら古いものから順に「実測 ID (+ 窓の offset) + 1 行要約 (関所が先頭 200 文字で作る)」に置き換える。記録は run dir に全文が残るので、役は ID で引用でき、窓は読み直せる
- モデルは結果の**抜粋**を読む (1 件あたり 32 KiB まで)。全文は記録に残る。**抜粋で切れた出力は、`{"read":{"id":"m-0001","offset":<byte>}}` で続きを 32 KiB ずつ読める** (関所が記録から切り出す。抜粋も窓も文字の途中では切らない。probe は走らず記録も増えない。回数は別枠で 1 巡あたり上限 120、`invocation.json` に `reads` として残る)。件数や有無を結論に使う出力は、読み切ってから書く。読めなかったときだけ「抜粋しか見ていない」と記録に書く (2026-09-07: イベント一覧 94 KB を抜粋だけで数えた報告が、全文を読む根拠レビューに 3 巡差し戻され不成立になった)
