...
func validateProof(files map[string]json.RawMessage, config worker.Config, record Record) (featureProof, error) {
	...
	if designDecision.Validate(identity, investigate.DesignSubject(design), designReviews, config.StageCeiling(), len(config.Models.DesignJudges())) != nil || worker.ValidateDesignBinding(candidate, design, worker.DesignDecisionSummary{Subject: designDecision.Subject, SubjectSHA256: designDecision.SubjectSHA256, Outcome: designDecision.Outcome}) != nil {
