# 調査・設計役 — 実物を計ってから、直し方を決める係 (設計書 v0.2、issue #18)
...
変更履歴: v0.1 (2026-09-04) 初版 → v0.2 (同日) 独立レビュー 2 系統 (敵対レビュー BLOCKER 3 / 3 層ゲート採点 FAIL 52 点) を反映。役を「プロセスを持たないモデル呼び出し」に改め、資格情報を資源単位で定義し、設計書を納品の関所に束縛した。
...
- `sql` は関所が自分で接続する (psql を起動しない)。**単文** (`;` を含まない) の SELECT だけ。`statement_timeout` と `default_transaction_read_only = on` を接続ごとに設定する。**本体の守りは DB 側の権限** (§3.3)。正規表現は費用と早期停止のためで、`SELECT pg_sleep(...)` のような遅い文は timeout が止める
...
1. **資格情報が書けず、見てはいけないものを返さない** — これが本体。資源と操作の単位で列挙する。動詞の接頭辞 (get/describe/list) で分類してはいけない: `get` は Secret の中身も、オブジェクトの本文も、鍵の発行も返す
   ...
   - DB: 読み取り専用ユーザーに、**顧客の内容を含まない列だけを持つビュー**への SELECT を GRANT する。表そのものへの権限は与えない。`dblink`, `postgres_fdw`, `pg_read_file` 系の関数は実行権限を REVOKE。接続は `default_transaction_read_only = on`
...
- **実装方針コメント** (既存 marker `plan`) の中身を、設計書の要約 (cause / approach / files / verification) に置き換える。**頼んだ人は、コードが書かれる前に方針を読める**
- 板 (状態ボード) の段階: `investigating` (調査中) / `designing` (設計中) / `design-review` (設計のレビュー中) を足す

## 5. AC3 — 設計レビュー (コードの前に方針を止める)

- レビュー役 A/B (別会社、既存の Hermes プロファイル) が `design.json` を判定する。判定の形は既存と同じ (`verdict: pass | revise`, `findings[]`。pass のとき findings は空、という既存の規則もそのまま)。1 人でも revise なら不成立
...
- レビュー役は `measurements.jsonl` を読める (probe を打つことはできない)
- 巡数: 消費側設定 `design_max_rounds` (既定 3。既存の `max_stages` と同じ場所に置く)。revise のとき役は**実測を撮り直さず**、同じ `measurements.jsonl` に追記して (`design-N+1`)、設計書を書き直す。既存の実測は不変 (追記のみ)。上限でも合意しなければ **正直に停止** し、争点を発注者への 2 択質問に変換する
...
| `internal/hook` | 調査報告コメント (marker `investigation`)、実装方針コメントの設計書要約 |
...
| 段階 | 内容 | 受入 (計れる形で) |
|---|---|---|
| 0 | probe 実行器・記録・秘密走査・確定記録の検算を**単体テストで固定** (稼働環境なし)。資格情報 3 本 (AWS 読み取りロール / DB 読み取りユーザーとビュー / K8s 読み取り SA) と probe カタログ。**中身を提示してから適用** | 故意の試験 8 件が全部拒否される: K8s の `secrets` get、`pods/exec`、AWS `secretsmanager:GetSecretValue`、`s3:GetObject`、DB の表への直接 SELECT と `UPDATE`、`dblink`、HTTP のリンクローカル宛 |
...
| 死因 | 固定するテスト |
|---|---|
| カタログ外の probe、正規表現に合わない穴、`;` や空白を含む値、リンクローカル宛の HTTP が実行される | probe 実行器: 実行せず拒否として記録 |
| 出力に鍵の形が含まれたまま保存・添付される | 秘密走査: 保存せず拒否として記録。添付前の再走査 |
| `measured` の finding が実測 ID を引用していない / 引用先が拒否 / `measurements.jsonl` の指紋不一致 | investigation の検算 |
| 設計書の `files[]` に許可範囲外のパス / `cause` に `measured` が無い / 文言の確認方法が変更前のファイルにある | design の検算 |
| 写し役が `files[]` に無いファイルを変えた | `seal-candidate` と `ValidatePublishGate` の包含検査 |
| `DESIGN.md` と `design.json` が食い違う | 描画は決定論 (同じ入力から同じ出力) をテストで固定 |
| 写し役の `revise-design.json` が無視されて候補が封緘される | 封緘段のテスト |
| 設計巡が上限で合意しないまま写し役に進む | design-decide のテスト |
| 起案役が「設計不要」と言い確認役が反対したのに飛ばす | needs_design の機械矯正 |
| probe の手数上限・壁時間で最終回答が無いのに `ready` 扱いになる | `investigation_incomplete` の終端 |
