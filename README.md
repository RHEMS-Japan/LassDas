...
本体は 1 つの課題につき最終報告のコメントを 1 回だけ残す (成功・失敗・調査のみ、どれでも)。受付はその課題に最終報告が既にあるかをトラッカーで確かめ、あれば `already_reported` として何もしない。台帳ではなくチケット側の印で判定するので、本体を作り直して台帳が空になっても、届いたばかりの webhook でも、起動時の取りこぼし回収でも、同じ課題に 2 本目の Pull Request が立つことはない。やり直したいときは新しい課題を作る (同じ課題に印を消して再投入する経路は用意しない)。
