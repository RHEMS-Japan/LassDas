...
- **同じことを繰り返したら、本体が裁定する。** 直前の巡と、指摘（座席・指摘コード・対象パス）が完全に同じか、書いたバイト列が同じパスに対して完全に同じ（検証まで走った巡なら、その結果も同じであること）なら、停滞とみなす。何巡続いたら裁定するかは `stagnation_repeat_rounds`（既定 1 = 直前の巡と同じなら、その場で）。裁定役はチケットの受入条件を読み、「レビューが依頼より多くを求めている」なら指摘を退け、「変更が依頼に足りていない」なら実装役に何を満たすかを指示する。**退けたときはその巡をもう一度判定し直し、足りないと裁定したときは次の巡が始まる。** どちらでも依頼者には何も聞かない
...
| `stagnation_repeat_rounds` | 納品先 | `1` | 何巡同じなら裁定するか |
...
| 停滞 | **確定 (2026-09-26)** | 直前の巡と指摘・変更が完全一致したら本体が裁定する（`stagnation_repeat_rounds` 既定 1）。依頼者には聞かない |
