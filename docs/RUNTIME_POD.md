...
| A retry that erases the attempt it was meant to answer — the second launch failing before it starts, or the card's wall firing during it, leaving no record at all | `cmd/worker` `TestTheRetryIsSkippedWithoutRoomOrTime`, `TestAnAgentThatChangedNothingIsAskedAgainWithTheTreeInFrontOfIt` (empty_attempts) | design review, 2026-09-09 |
