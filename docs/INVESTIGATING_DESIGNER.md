...
確認方法は **1 つの文言 (wording: path + expected_text 1 つ + absent_text 1 つ) か 1 つの計測 (measurement)** しか持たない。反映後に関所が自動で計るのは measurement 形だけで、wording 形は実装役への指示と受入時の表示確認の目安になる (staging の文言検査は受付が導いた `readiness-ticket.json` の文言で行う)。`absent_text` は **設計ファイルが基線で現に持つ文言** に限る (関所は基線で存在を検算する)。設計ファイルが全部この変更で作られるときは空にする。`absent_text` は任意で、設計ファイルが全部新規のときは空でなければならない (v0.22: レビュー役にもそう伝える)。設計を判定するレビュー役 (根拠・方針の両レンズ) はこの書式を知らされ、この変更の成否を判定できる別の確認方法が書式内で書けるときだけ確認方法を理由に revise する (複数項目の有無や文書全体の内容の検査は書式で表せないので revise の理由にしない)。→ v0.21。
