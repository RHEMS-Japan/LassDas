<!-- evaluate: 90/100 | blockers: 0 | stage: review | type: feature | date: 2026-09-04 -->
<!-- score provenance: independent 3-gate scoring of v0.7 (commit 3257ba5) = 90/100, Gate 1 PASS; v0.8-v0.10 fold that pass's residual small items and the adversarial reviewer's final round (BLOCKER 0 / MAJOR 0 at v0.6). History: v0.1 52 (3 blockers) -> v0.2 78 -> v0.3 79 -> v0.4 84 -> v0.7 90. -->
# 調査・設計役 — 実物を計ってから、直し方を決める係 (設計書 v0.10、issue #18)
...
| 2 | 設計モード + 設計レビュー + 写し役。**#23 を前提**。無害な変更依頼を **5 本** 流す | 5 本のうち、方針の誤りが実装後に見つかる件数 (§5 の 3 信号) **0**。写し役が手数上限 40 に到達した件数 **0**、手数の中央値 **20 以下** (写し役は native カードで関所は会話を見ないので、封緘するカードが run dir に残る Hermes のセッション記録から手数を数えて `candidate.json` の `applier_turns` に刻む。記録が無ければ「計測不能」と刻み、DoD の分母から外す)。設計 1 巡の費用が実装 1 巡の費用を超えない (実測で記録) |
