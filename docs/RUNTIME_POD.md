...
- the gateway's billing reading (`spend.json`, written by every terminal report: the same figures the terminal comment carries, per key with the roles it serves and the key's name, never its value — so a failed run's cost shows too).
