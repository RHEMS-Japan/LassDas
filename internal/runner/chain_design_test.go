...
func TestDesignRoundsAreCountedFromSealedDecisions(t *testing.T) {
	...
	for _, want := range []string{"You apply an approved design", "# Design — round 2", "revise-design.json", "Change only the files the design lists", "the root of this repository", "1 to 600 bytes", "There is no person on this run", "write the first change early", "remove anything you created", "blast_radius|not_doing", "Decide before you edit"} {
