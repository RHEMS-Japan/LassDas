...
**自分のプロジェクトに合う AI との開発ワークフローを構築し、仕事を任せられるようにするフレームワーク。既存の進め方があれば取り込み、なければ LLM と相談しながら作る。**
...
4. 本体イメージは公開レジストリ (ghcr.io) にあるのでログインは不要。案内に `registry_login` があるときだけ、そのコマンドを **利用者が** 実行する。AI は鍵を扱わない
...
この一文がすべての設計判断の親である。ここから 5 つの原則が導かれ、フレームワークの各機構はいずれかの原則の実装である。

1. **責任を取る瞬間は、少なく、明確に。** 人間の出番は 3 点 — 依頼を書く（何を頼むかの責任）、質問に答える（仕様の責任）、Go を出す（反映の責任）。承認点を増やすほど人は読まずに押すようになり、責任は形骸化する。それ以外の全工程で人間を煩わせない。
2. **見えないものに責任は取れない。** 責任を取らせるなら、見せる義務は AI 側にある。全報告はチケットに残り、証拠（スクリーンショット・検査結果）が付き、状態ボードで「今」が見える。可視化は付加機能ではなく、責任の前提条件である。
3. **果たせない承認を求めない。** 押しても成立しない承認ボタンを人間に出すことは、責任だけ渡して権限を渡さない欺瞞である。承認を求めるのは、承認が必ず履行される状態に限る。
4. **不明は不明と言う。** 「反映されたか確認できなかった」は、そのまま報告する。責任者への虚偽報告は、人間から責任を取る能力そのものを奪う行為であり、この体系における最悪の罪である。
5. **責任には拒否権が伴う。** 人間はいつでも止められる。止める権利のない人間に、責任を取らせることはできない。
...
手元で全チケットの現在地と経緯を一覧する画面。正本 (状態の台帳・トラッカー・実行基盤) から都度組み立て、独自の記録を持たない。

書き込みはただ 1 つ: エンジンが投げた確認質問への回答をボタンで選び、質問コメント自身が印字した「回答 ...」行だけを本人名義のコメントとして投稿する。自由文は投稿できない。回答機能が現れるのは、`LASSDAS_CONSOLE_TRACKER_KEY` の持ち主 (起動時に `users/myself` で実測) がそのチケットの許可回答者 — エンジンが回答を採用する唯一の相手 — と一致するときだけで、一致しなければ画面は理由を示して Backlog での回答を案内する。質問の同定はエンジン自身のコメント目印 (本文最終行) に委ね、本文中に目印形の文字列が現れても質問とは扱わない。

```
LASSDAS_CONSOLE_STATE_TABLE=<台帳テーブル名> \
LASSDAS_CONSOLE_TRACKER_DOMAIN=<your-space.backlog.com> \
LASSDAS_CONSOLE_TRACKER_KEY=<閲覧と回答に使う API キー> \
...
導入は、利用者がすでに使っている開発 AI (Claude Code や Codex) に、この repo の URL を渡して頼む (冒頭の手順)。AI が [docs/SETUP.md](docs/SETUP.md) に従って repo を調べ、決めることを聞き、`.lassdas/` に合意と回答を書き、`lassdas setup` で本体を起動し、利用者が本人の鍵で試験依頼を 1 本流して完了になる。main の image workflow が公開イメージを作り、`lassdas setup note …` で [docs/DISTRIBUTION.json](docs/DISTRIBUTION.json) を更新する。旧来の対話ウィザード `lassdas init` も残っている。シンプルな CLI アプリは、Apple Silicon の Mac と Docker Desktop で始める。ソース版には Git と Go (`go.mod` の版) も必要。
...
GitHub・トラッカー・モデルの鍵は、それぞれのサービスで外から取得して入力する。初期版のモデル接続先は OpenRouter 固定で、キー1本を全役で共用する。URL の入力は不要で、役ごとの別キーは任意で選べる。init は納品先の検証、設定生成、本体起動、本人名義の動作確認を順に進め、最初の PR と工程の記録が一致して完了になる。中断後は同じコマンドで再開する。
...
> 現在地: 第一段階・第二段階とも「運用コンソール」節の形で実装済み。認証は Backlog OAuth ではなく、起動時に渡した API キーの持ち主を `users/myself` で実測し、台帳が記録する起票者と一致する場合だけ回答を有効にする方式を採った — 127.0.0.1 専用ツールではキーの所持が本人性そのもので、redirect を伴う OAuth はローカルサーバーには過剰なため。投稿はそのキーの持ち主名義の新規 Backlog コメントで、本項の「本人の credential により本人名義で投稿」の要件を満たす。
