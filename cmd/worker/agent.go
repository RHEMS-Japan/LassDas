...
func reviewAgentPrompt(
	...
	sections = append(sections,
		...
		"- 指摘と提案する直し方の両方を、該当する実装・依存先・記録で検算してください。反証となる処理も確認し、message に根拠のファイル・箇所または実測を示してください。設定上の上限やログ名だけから、実際の所要時間や処理の完了を断定しないでください。",
		"",
		"## 調査の予算 (超えると失敗扱い)",
...
func implementPrompt(
	...
	if len(findings) > 0 {
		...
		sections = append(sections,
			...
			"### 前回の指摘 (根拠を検算して対応すること)",
			encoded,
			"- 前回の指摘は検証対象です。引用された実装・依存先・記録を読み、前提と提案された直し方が正しいかを確かめてください。正しい部分を修正し、誤りが分かった指摘の主張を成果物へ取り込まないでください。退けた指摘は、その根拠を最後の報告で説明してください。",
		)
		if omitted > 0 {
			sections = append(sections, fmt.Sprintf("- (指摘が多いため先頭 %d 件のみ掲載、%d 件省略)", len(findings)-omitted, omitted))
		}
