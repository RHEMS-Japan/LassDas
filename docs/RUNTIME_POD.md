...
| A gateway answer of 429/502/503/504 on one of an investigation's dozens of calls ending the run outright | The call is posted again after a pause, up to three times; every other status fails closed at once | `internal/worker` `TestGatewayClientAsksAgainAfterAGatewayTimeout` | live, 2026-09-08 |
| The reception presenting measured values and measurement record numbers it never obtained as choices, and the chosen one preserved as the requester's answer | Questions, choices and assumptions that cite a record are refused as invented; the assessor is told it measures nothing and the checker has a defect code for it | `internal/worker` `TestReadinessRefusesFabricatedMeasurements` | live, 2026-09-08 |
