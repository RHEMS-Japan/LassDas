...
追記 (2026-09-09): 発注者の指示により、ローカル起動の板は Basic 認証不要の閲覧用へ変更。設定生成・起動検査・利用手順をこの契約へ合わせる。Pod の既定の認証方式は維持する。上記の独立評価点は変更前の v0.2 の記録で、本追記の評価ではない。
...
lassdas init
 ...
 ├ 5 本体      設定・env・volume → 起動時検査 → 認証なしのローカル板を確認
...
| 5 本体 | 設定・保存先・板の接続先をまとめて確認 | §5 の設定読込、必須秘密ファイルの実在、別ユーザ起動、読取拒否、常駐の生存、ローカル板の認証なしでの閲覧。ログの一語だけで成功判定しない。既存コンテナのラベル・設定指紋を照合し重複起動しない |
...
statusboard はコンテナ内 :9200。init は `LASSDAS_BOARD_AUTH=local` を生成し、Basic 認証なしの閲覧用とする。user/password は生成しない。host は `-p 127.0.0.1:9200:9200` のみ。使用中なら空き port を提示して選択を保存。hermes serve の 127.0.0.1:9119 とは別。local モードは loopback の Host 名だけを許可し、操作の投稿と webhook を無効にする。Pod と既存の Basic 設定は user と16文字以上のpasswordを引き続き要求する。

ローカル板の試験対象は `/` と `/api/board`。認証なしで両方を読めることを確かめる。`/healthz` だけで起動成功としない。既存の Basic 設定では `/` の未認証拒否と認証成功を引き続き検査する。
...
LASSDAS_BOARD_AUTH=local
```

ローカルの閲覧用設定では board-tracker-key と board-pass を生成しない。既存の Basic 設定では board-pass が必須、board-tracker-key は操作用鍵を設定した場合だけ存在する。target-token はどちらでも必須で、存在して関所から読めることも確認。既存 guard は無いファイルを飛ばすため、一覧だけでは不十分。初回にできる route.key は生成後も検査し、以後の boot では同じ一覧で検査する。
...
| 1: 立ち上げ (B/C/E) | §1.1 の前提を揃えた macOS/arm64 の新規 project で設定の手編集なしに起動。起票者・検証場所・秘密保護の 3 ブロッカーを解消。中断/失効/再開で重複資源なし。ローカル板の認証なしでの閲覧、秘密欠落/漏えい時の未完了。入力欄数と時間を記録 |
...
| ログだけで成功を誤認 | ローカル板の閲覧・本体の生存、PR の repo/base/head/差分と確定記録を実際に照合 | C/D |
