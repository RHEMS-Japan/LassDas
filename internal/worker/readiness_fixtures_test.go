...
func TestReadinessFixturesAreExecutableAndGateCorrectly(t *testing.T) {
	for _, fixture := range loadReadinessFixtures(t) {
		t.Run(fixture.Name, func(t *testing.T) {
			...
			assessment, _, err := invoker.AssessReadiness(context.Background(), 1, nil, nil, clarification, nil, source, request, config, nil)
			if err != nil {
				t.Fatalf("AssessReadiness() error = %v", err)
			}
			check, _, err := invoker.CheckReadiness(context.Background(), assessment, clarification, nil, source, request, config, nil)
			if err != nil {
