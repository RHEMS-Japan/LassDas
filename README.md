...
**いま何が動いていて、何が残っているか**は [docs/HANDOVER.md](docs/HANDOVER.md)。**いま動いている契約**（巡・裁定・再試行・質問・Go・終端条件とその設定）は本ファイルの [いまの契約](#いまの契約実装済みの動き)。本ファイルのそれ以外の節には「要設計 / 設計提案」の未決事項が混ざっており、契約ではない。
...
1 ステージでは、同一の成果物を設定済みの複数 AI へ同時に渡して独立に敵対レビューさせる。修正対象の指摘があれば新しい成果物を作って次のステージへ進む。**繰り返す回数に既定の上限は置かない** — 進んでいるあいだは続け、直前の巡と同じことを繰り返したら本体が裁定して先へ進める（[いまの契約](#いまの契約実装済みの動き)）。収束すれば納品先の設定どおりの深さまで届け、届かなかったときは各ステージの指摘・修正・未解決事項をレポートして終わる。**本番へ反映する具体的な経路は要設計**であり、現時点では消費 repo の既存リリース経路を使う案を最小候補とする。
...
本体は 1 つの課題につき最終報告のコメントを 1 回だけ残す (成功・失敗・調査のみ、どれでも)。受付はその課題に最終報告が既にあるかをトラッカーで確かめ、あれば `already_reported` として何もしない。最終報告は本体が自分で書いた (投稿者がその本体の鍵の持ち主である) コメントだけを数える。人が書いたコメントの末尾がたまたま同じ形でも報告とは見なさない。逆に、別のトラッカー利用者名義で動く本体が出した報告は見えないので、その課題はもう一度処理される (2 本目の Pull Request は閉じればよい。見えない課題が永久に無視されるより安全側に倒している)。

手元の台帳はその本体が自分でやったことしか知らない。別の本体が納品した課題や、その本体が止まっている間に立った課題は台帳に無いので、取りこぼし回収がもう一度拾ってしまう。チケット側の印で判定するのはこのため。判定できるのは**最終報告まで終わった課題**で、納品の途中 (Pull Request は出したが最終報告の前) に取り込み直された場合は防げない。やり直したいときは新しい課題を作る (同じ課題に印を消して再投入する経路は用意しない)。

> **設計ドラフト**: 本番昇格の出口と、敵対レビューを反復して収束を判定することは合意済み。収束条件、修正担当、判定者の必要数、本番反映後の完了条件は未設計であり、本 README で確定事項として扱わない。巡の回数・停滞の裁定・再試行・質問・Go については実装済みの契約があり、[いまの契約](#いまの契約実装済みの動き)がその正本である。
>
...
| 本番昇格ゲート | 複数 AI の敵対レビュー → 修正を、進んでいるあいだ繰り返す。同じ巡の繰り返しは本体が裁定する（実装済み。[いまの契約](#いまの契約実装済みの動き)） |
| 収束条件 | **要設計** |
...
**受入目標との差（2026-09-26）:** 夜に任せた依頼が朝には要求を満たして納品されていることが目標であり、失敗報告・再試行の打ち切り・同じ処理の無限反復は完了ではない。以下には期限切れや内部エラーで終わる現行の動きも記載しているが、これらは目標を満たす例外ではなく、解消すべき未達事項である。今回の答えの読み取り修正だけで、納品までの保証が成立したとは扱わない。

**この節が現在の契約の正本。** ここに書いてあるのは設計案ではなく、いま動いているものの説明である。以降の節には「要設計 / 設計提案」と明記した未決事項が混ざっており、そちらは契約ではない。食い違いを見つけたら、この節が正しい。

### 受付を通ったあとは、人に何も聞かない

保証の始点は**受付を通ったとき**である。受付とは、起票を拾ってから着手可否の判定（readiness）を通過するまでを指す。

- **受付の手前では、質問が 1 巡だけ起こりうる。** 残った疑問を 1 つのコメントにまとめて一度だけ聞く。件数は `question_max_items`（既定 10、上限 19）、聞ける回数は `question_max_rounds`（既定 1）。`questions` を `none` にした納品先では 1 問も聞かず、受付がすべて自分で判断して前提として記録する
- **受付を通ったあとは、誰にも何も聞かない。** 実装役が仕事を返してきても、レビューが噛み合わなくても、工程が失敗しても、本体が決めて進める。決めたことは【実装方針】コメントと最終コメントに前提として出る

実装役が何も変更せず報告した場合の再指示は、元の依頼・設計・変更範囲・検収条件・納品先を変える許可ではない。既定を選べるのは未指定の実装詳細だけであり、変更禁止の条件を破る編集や、必要な実接続・本番納品の代わりの fake を要求しない。旧版が保存した再指示も、次の起動では現在のこの制約で作り直し、過去の記録は変更しない。不足は実装役の未検証の報告として残し、代用品の実装済み・運用者による後日の供給を断定しない。これは復旧の指示の修正であり、何も変更していない報告を成功にする変更ではない。元の条件に従った「変更なし」の完了を独立に検証する経路は未実装。

例外は 2 つだけで、いずれも**運用者が設定でそう選んだときにしか起きない**。

| 例外 | 設定 | 既定 | 選んだときに起きること |
|---|---|---|---|
| 本番の前に人の Go を待つ | `chain.deliver.go_gate` | `off`（待たない） | `required` にすると、staging の確認が通ったあと、チケットに「Go」と書かれるまで本番へ進まない。待つ上限は `chain.deliver.go_wait_seconds`（既定 7 日）で、過ぎたら本番未反映を報告するが、本番までの依頼を成功終了にはしない。期限切れからの再開は未対応 |
| 工程の再試行に上限を置く | `chain.retry_max_attempts` | `0`（上限なし） | 値を入れると、1 つの工程が同じ巡でその回数だけ配り直された時点で、はしごの分類が持つコードで終わる（納品の段なら `release_failed`）|

**人が張り付かない納品先では、どちらも既定のままにする。** `go_gate` を `required` にした時点で、夜に投げた依頼は朝まで人を待つ。`retry_max_attempts` に値を入れた時点で、一時的な障害がそのまま終端になる。

### 納品の途中を成功と呼ばない

**カード方式では、要求された納品地点と、実際の納品記録が一致してから成功にする。** PR までの依頼は PR、統合までの依頼は staging、本番までの依頼は本番の確認が必要。PR ができたことや staging が通ったことだけで、本番までの依頼を閉じない。

最初の納品カードを作る前に、対象リポジトリと要求された地点を `delivery-depth.json` に保存する。経路の設定が不足している間も依頼と証拠を保持し、設定された経路が戻れば同じ依頼の納品を続ける。設定が読めない、保存済みの地点と一致しない、記録が壊れた・保存できない場合は新しい納品カードも成功報告も出さない。設定の変更で保存済みの地点を縮めたり、別リポジトリへ変更したりしない。PR 作成前の受付時点からこの地点を固定することは、まだ保証していない。

本番用 workflow や staging の反映証拠が不足している場合も、確認できるまで本番へ進めず、成功にはしない。Go、反映を有効にした時刻、検証、明示的な停止は維持する。すでに旧版で送信を開始した最終報告は、二重投稿や照合不一致を避けるため同じ内容で再送する。終了済みの依頼を起こし直すことはしない。

**保持は完成ではない。** 恒久的な設定不足や失われた記録を自動で作り直すこと、保存済みの昇格保留・Go の期限切れから再開すること、復旧待ちの詳細を盤面やチケットに示すことは未達。今回の試験は台帳・納品カード・コメントの試験環境での復旧であり、実サービスへの反映を実証したものではない。

### AI の答えは、形が違っても読む

**AI の答えは「本体が必要な項目」だけを読む。** 知らない項目名が付いていても、JSON の前後に説明文やコードフェンスが付いていても、同じ項目名が 2 回書かれていても、答えとして読む。読めないのは、JSON が 1 つも書かれていないときと、本体が使う項目の値が型ごと違うとき（文字列のはずが配列など）だけである。

**形で答えを捨てないのは、捨てると依頼が死ぬから。** 実測（2026-09-26）: 情報が足りない依頼 3 件が、受付の 2 分後に内部エラーで終わった。原因は AI ではなく本体で、受付の読み取り役が質問項目に本体の知らない項目名を 1 つ添えたために答えごと捨てられ、3 回聞き直して終端した。**強いモデルに替えても同じところで終わった。** 質問を 1 巡だけ出す経路が要るのはまさにこういう依頼で、それを殺していたのは本体自身の厳しさである。

**安全性の検査は 1 つも緩めていない。** 触ってよいパスの範囲、URL の許可リスト、鍵の名前、生成物に混ぜてはいけない文字列、サイズの上限は、読み取ったあとに今までどおり適用する。緩めたのは**答えの形の厳しさだけ**で、答えの中身に対する検査ではない。

- **本体が自分で書いた記録と、運用者が書いた設定は、今までどおり厳格に読む。** 知らない項目があれば拒む。別のバージョンの本体が書いた記録を「読めたつもり」で扱わないためである
- **工程（実装・レビュー・検証）の答えが読めなかったときは、その座席の失敗になる。** はしごが呼ばれ、別のモデル・別の提供元へ移し、移せる先が尽きたら短く聞き直し、それでも駄目なら倍々で待つ。依頼は終わらない

### 受付の読み取り役が答えなかったら、本文をそのまま渡す

**受付（依頼の読み取りと着手可否の判定）は、はしごの上に無い。** 別の座席へ移す先も、待って変わるものも無い。それでもここで終わらせない。

- **3 回聞いても読める答えが返らなかったら、本体が自分で読む。** 依頼内容は**チケットの本文そのまま**。読み取り役の代わりに仕様を作り足さない
- **依頼は実装役へ進む。** 何を変えるかは実装役がリポジトリを読んで決める。受付の読み取り役が本来補うはずだったものは、そのリポジトリの中にある
- **起票者には 1 行で伝わる。** 【実装方針】コメントと最終コメントの前提の欄に、本文をそのまま採ったことを記録する。読み取りだけを代替した段階では、この後の受付判定が質問する可能性があるため「質問していない」「実装へ渡した」と先取りしない。着手判定も代替した場合は、質問せず実装へ進む判定を記録する。同じ代替の記録は依頼 1 件につき 1 行だけ
- **読める答えが混ざっていればそれを使う。** 説明文に埋もれた JSON から質問項目が読めたなら、最初からきちんと書かれていた場合と同じに扱う

**上の実装への引き渡しは、納品先が確定できた場合の動き。** 複数の納品先を設定していて対象を確定できない場合は、実装には渡さず `clarification_required` で報告する。納品先を選ぶ質問を投稿して回答から再開する処理は未対応であり、「質問を投稿した」「実装役へ渡した」とは記録しない。これは納品完了ではない。

受付・実装候補・裁定・調査・設計・レビュー・接続試験が形式例を先に書いたときは、各役の答えを識別する項目を持つ**最後の完全な JSON オブジェクト**を読む。後に付いた無関係のメタデータは答えにしない。走査量の上限に達した場合や本回答の型が違う場合、前の例で代用しない。検算の答えが読めず判定を作り直した場合、破棄した判定の前提は実装方針や最終コメントに混ぜない。

受付が「着手可能」と書きながら質問を添えた場合も、質問の本文・選択肢を捨てない。質問があれば確認の巡へ、確認すると書いていても質問が無ければ着手可能へ読み替える。質問番号はこの回答内の順に付け直す。保存済みの判定にはこの読み替えを適用せず、署名対象の内容を検査する。

**この経路に入るのは「答えが使えなかった」ときだけ。** 提供元に拒まれた、鍵の利用枠を使い切った、通信が届かない、保存先に空きが無い — これらは運用者が直すものなので、今までどおり終端する。本文をそのまま渡して進んでしまうと、直すべき故障が「進んだ依頼」の陰に隠れる。

### 内容を理由に依頼を突き返さない

**読める依頼を、内容を理由に受け付けないことはない。** 依頼を弾けるのは処理できない入力だけ — 空、大きすぎる、UTF-8 として壊れている、制御文字が入っている。これは本文そのものに対する機械的な検査で、モデルが読む前に済んでいる。

**受付の読み取り役が「受け付けない」と答えても、それは終端にならない。** 本体が実際に取れる 2 つの行動のどちらかに読み替える。

| 読み取り役が答えたもの | 起きること |
|---|---|
| 「受け付けない」+ 聞くべき点も書いた | その質問を 1 巡だけ出す。書かれた質問・選択肢・既定はそのまま使い、1 つも落とさない |
| 「受け付けない」+ 聞くべき点は書かなかった | 依頼はチケットの本文のまま実装役へ進む。【実装方針】コメントと最終コメントの前提の欄に「受付の読み取り役はこの依頼を受け付けないと判断しました (理由)。依頼者に確認すべき点は挙げられなかったため、依頼を突き返さず、チケットの本文をそのまま実装役へ渡しています。」が 1 行出る |

**何に難色を示したかは、両方に届く。** 読み取り役が使った語は判定の記録に封じられ、運用者はそれをそのまま読める。起票者には上の 1 行に理由の節が入る (例: 「本体に渡されていない権限や設定がないと果たせないと見たため」)。**起票者に機械の語は出さない** — 本体が文を持たない語のときは「理由は実行の記録に残しています」と書く。

拒否理由が空でも、符号ではなく文章でも「読めない答え」とは扱わない。文章は従来の 64 バイト上限と文字の検査の範囲で記録し、理由が無ければ「理由は示されていません」と伝える。

**実測 (2026-09-26)**: リポジトリに無い仕様書を名指しした依頼が「受け付けない」と判定され、質問も理由も記録されないまま「チケットの内容が自動処理の受付条件を満たさなかった」で終わった。次に行動する人は運用担当者とされ、運用担当者が説明すると書かれていたが、それを送る仕組みは無い。**起票者にも運用者にも、やることが 1 つも無い終わり方だった。**

**`readiness_rejected` で終わる依頼は、もう出ない。** 内容を理由に突き返す経路が無くなったので、いま動いている経路でこの名前が付くことはない。名前と最終コメントは、以前のバージョンが封じた判定を読み直したときのために残してある。そのコメントは「本文が読み取れる形ではなかった」という機械的な理由を 1 文で述べ、次に行動する人を**起票者**とする (読める本文で起票し直す)。

現行の入力の読み取り検査は `input_rejected` で通知し、入力が読めなかったことと起票者が行う確認を記載する。契約を作る段の未回答事項はこれと区別し、入力不正と呼ばない。

### 巡は、進んでいるあいだは続く

- **巡の数に既定の上限は無い。** `max_rounds` の既定は `0` = 無制限。レビューが指摘し、次の巡でそれが直り、また指摘が出る — 進んでいるあいだは続く。運用者が数を入れたときだけ、その数で止まる
- **同じことを繰り返したら、本体が裁定する。** 直前の巡と、指摘（座席・指摘コード・対象パス）が完全に同じか、書いたバイト列が同じパスに対して完全に同じ（検証まで走った巡なら、その結果も同じであること）なら、停滞とみなす。さらに、**一度消えた同じ座席・コード・パスの指摘が戻る往復**と、途中で変えた内容が以前の不合格と同じバイト列・検証結果へ戻る往復も裁定する。他の指摘が増減していても検知するが、指摘が残り続けたまま他が減るだけの場合を往復とはしない。間の巡が読めなければ、指摘が消えたとは判定しない。`stagnation_repeat_rounds`（既定 1）は連続一致の回数、または同じ指摘・内容への戻りの回数。裁定役はチケットの受入条件を読み、「レビューが依頼より多くを求めている」なら指摘を退け、「変更が依頼に足りていない」なら実装役に何を満たすかを指示する。**退けたときはその巡をもう一度判定し直し、足りないと裁定したときは次の巡が始まる。** どちらでも依頼者には何も聞かない。検知しただけで合格にはせず、変更範囲・検証・納品の条件も変えない
- **記録の天井は 50 巡。** これは予算ではなく、巡番号を持つ記録すべてに掛かる同一の上限である。ここに当たった依頼は、その時点の分類が持つコード（`nonconverged` / `validation_failed` など）で終端し、最終コメントを出す
- **設計の巡には、まだ裁定が無い。** 裁定が効くのは実装の巡だけで、設計の判定役が指摘を出し続ける依頼は記録の天井 50 巡まで回り、そこで `design_nonconverged`（調査なら `investigation_nonconverged`）で終わる。`design_max_rounds`（既定 3）はその天井の計算に参加するだけで、設計を打ち切る数ではない。**設計側の裁定は未実装**
- **`max_stages` はもう巡を止めない。** 既存の設定ファイルを書き換えずに済ませるために欄として残してあり、いまは 1〜5 の必須欄として検証され、記録の天井の計算に参加するだけである（1〜5 は 50 を超えないので、天井は動かない）。上限としては働かない

### 裁定には、すでに試したことを渡す

裁定役には現在の候補・指摘・依頼に加え、**以前の巡の変更理由・対象パス・レビュー・裁定・検証の失敗**を渡す。同じ失敗した方法を再び指示するなら、今回は何の根拠が違うのかを示させる。リポジトリの事実が足りない場合は、許可された範囲内の具体的な代案を調べるよう実装役へ指示する。過去の裁定を理由に変更範囲や合格条件を緩めることは認めない。

履歴は既存の実行記録を検査して読み、別の依頼や壊れた記録は「利用できなかった巡・証拠」として区別する。それだけで現在の裁定を止めたり、過去の巡を成功と扱ったりはしない。履歴が 64 KiB に収まらない場合は新しい巡を優先し、省いた巡番号を明記する。渡した履歴は裁定記録にも封じる。過去の指摘は参考情報で、退けられるのは現在の巡に実在する指摘だけである。

裁定役は、基準コミットに一致する未変更の checkout に対し、既存の `repo.list` / `repo.read` / `repo.grep` と記録の続きを読む操作で、争点の事実を調べられる。候補の内容と基準コードは区別して渡す。裁定 1 回につき調査 8 回・記録の窓 8 回までで、実際に見せた観測は 64 KiB 以内で裁定記録にも封じる。取得した出力は既存の上限・秘密の除去を通して計測記録にも残る。切れた・拒否された・見せられなかった出力を「存在しない」という証明には使わせない。シェル・外部接続・編集の道具は渡さず、設計工程を有効化する設定変更や新しいモデル役も要らない。

裁定は既存の**検証カード内**で行い、進行係はモデルの応答を待たない。停滞した旧カードを見つけた場合も、同じ巡の検証カードへ渡して戻る。裁定中の停止受付と、他の依頼を進める tick はモデル待ちから切り離される。裁定の中断・失敗は工程の復旧対象として残し、裁定できなかったことを同じ実装のやり直しに置き換えない。裁定が指摘を退けても、判定の再計算と実際の検証は省略しない。カードの再実行は保存済みの裁定から再開する。

裁定役のモデル失敗・時間切れ・残高不足では、`models.arbiter.candidates` の設定済み候補を一度ずつ試す。裁定役を省略した設定では、既存どおり受付の読み取り役とその候補を使う。この役はモデルへ直接問い合わせるため、候補用のエージェント起動設定は不要。設定の内容・鍵・課金上限は変更せず、実際に問い合わせた候補と呼び出し記録を裁定に封じる。通常の検証エラーでは裁定役を替えず、未実装の「短い聞き直し」を実行済みとも記録しない。

旧カードや中断後の復旧で最終巡にいる場合も、停滞を検知した候補の未実施の裁定と、保存済みだが未反映の裁定の再検算は、同じ検証カードで完了させる。回数上限は次の実装巡を増やすことを止めるものであり、現在の候補を判定し直すことまで省略しない。判定済みの裁定を繰り返したり、上限を越える実装巡を作ったりはせず、途中で読んだ停止指示も失敗終了に置き換えない。

これは有効な解決策を必ず見つける保証ではない。裁定自体の上限は最長 2 分のままで、復旧の手が無ければ既存の待機へ進む。実地での収束・停止・納品は未検証であり、回数・期限による失敗終了の経路も残る。

### 工程が失敗したら、はしごを降りる

工程（実装・レビュー・検証・納品）のカードが失敗したとき、失敗の種類を読み、**まだ試していない手を 1 つ打ってから配り直す。** 同じ手は二度打たない。打った手はボリュームの記録に残るので、実行体が入れ替わっても途中から続く。

| 段 | すること |
|---|---|
| 0 | ボリュームの空きを作る |
| 1 | 役を別のモデル・別の提供元へ移す（`models.<役>.candidates`）。移せる先が尽きたら、同じ相手に短く聞き直す |
| 2 | 同じものへ別の経路で届く |
| 3 | 足りない道具を別の場所から持ってくる |
| 5 | 待つ。間隔は倍々で、`chain.retry_backoff_base_seconds`（既定 60 秒）から `chain.retry_backoff_max_seconds`（既定 30 分）まで。`chain.retry_notice_attempts`（既定 3）回目に、まだ動いていることと何を待っているかをチケットへ 1 度だけ知らせる（返事は待たない） |

**検証の失敗ははしごに来ない。** 決定的検査が変更を拒んだのは変更についての答えであって工程の故障ではないので、その巡が拒否の出力を次の巡の指示に載せる。はしごは呼ばれない（段 4 の番号はその経路のために空けてある）。

**残高・利用上限による拒否も、設定済みの別の起動先を先に試す。** コードレビューと設計レビューでは、他の判定役と提供元が重ならず、独自の起動設定を持つ候補を一度ずつ試す。鍵の値は比較せず、上限を上げたり新しい鍵を探したりもしない。複数の起動設定が同じ口座を使う場合もあるので、移動しただけで復旧したとは扱わない。候補が無い・すべて拒まれた場合は待ち、残高不足を同じ相手への短い聞き直しで直そうとはしない。実装役など、候補の起動をまだ持たない役にはこの移動は効かない。

**待ちは完了ではない。** 再試行が成功すれば再開するが、現行実装では `retry_max_attempts` と次節の実行全体の期限による終端が残る。候補への移動だけで無人完遂を保証したとは扱わない。

### 実行全体の期限

**`run_deadline_hours`（既定 8 時間）。** 受付の回答待ちを除いて 1 つの依頼がこの時間を超えたら、「ここまでやった」— 到達した状態・失敗の種類・再開に必要なもの — を最終コメントにして終わる。永久に直らない障害でも、朝にはチケットに最終コメントがある。

### 内部エラーで終わる条件

次は現行実装に残る `internal_failed` の例であり、受入目標を満たす例外ではない。設定を変えずに受付だけ繰り返しても解消しない。これ以外にも準備・読み込み処理の終端が残っているため、全終端をこの表だけで網羅したとは扱わない。

| 条件 | 依頼者に見えるもの | 運用者がすること |
|---|---|---|
| 設計が要ると判定されたのに、この実行環境に設計工程の profile が無い | 「内部エラーが発生し、依頼を完了できませんでした」「次に行動する人: 運用担当者」「自動再試行: なし」 | `chain.profiles` の `investigate` / `design_review_a` / `design_review_b` / `design_decide` / `applier` を揃えて、依頼を出し直してもらう |
| 設計が要ると判定されたのに、納品先が `agents.applier` を持たない | 同上 | 納品先の設定に `agents.applier` を足して、依頼を出し直してもらう |

**受付済みの判定記録は、受付をやり直さず復元する。** 工程を選ぶ前に、依頼・設定・元コードへ結び付いた判定の写し (`accepted-reception.json`) を保存する。作業用の記録が壊れた場合は元の判定へ戻し、壊れた記録も別名で残す。古い走行で写しが無ければ、検算済みの読み取り・確認記録から再導出できる場合に復元する。判定役が質問を前提へ変えた答えも、写しからそのまま復元し、質問し直さない。

カード作成前の中断も同じ判定から引き継ぐ。復元できない間は理由をログに残し、依頼・カード・証拠を保持して次の巡回で復元を試す。**失われた判定役の答えを創作したり、空の記録を「設計不要」と読んだりはしない。写しも検算済みの材料も使えない永続的な障害から、自動で納品まで進めることは未達であり、待機を完了とは呼ばない。** 古い版が実際に受付をやり直した記録は、引き続きその事実を前提欄に示す。

**AI の答えの形は、もう終端の理由に入らない。** 以前は受付の読み取り役の答えを 3 回読めないと `internal_failed` で終わっていた。いまは本体が自分で読み、チケットの本文をそのまま実装役へ渡して進む（[AI の答えは、形が違っても読む](#ai-の答えは形が違っても読む)）。この変更は、保存先や設定の故障まで直したことを意味しない。

`release_failed` も同じ扱いで、`retry_max_attempts` に値を入れた納品先でだけ起きる。

### 設定の一覧と既定

| 設定 | 場所 | 既定 | 効き目 |
|---|---|---|---|
| `max_rounds` | 納品先 | `0`（無制限） | 巡の上限。運用者が数を入れたときだけ止まる |
| `stagnation_repeat_rounds` | 納品先 | `1` | 連続一致または同じ指摘・内容への戻りが何回なら裁定するか |
| `max_stages` | 納品先 | 必須（1〜5） | 巡は止めない。記録の天井の計算に参加するだけ |
| `design_max_rounds` | 納品先 | `3` | 設計の巡の記録の天井に参加する。設計を打ち切る数ではない |
| `questions` | 納品先 | `minimal` | 受付が聞いてよい範囲。`none` は 1 問も聞かない |
| `question_max_items` | 納品先 | `10`（上限 19） | 1 コメントに載る質問の数 |
| `question_max_rounds` | 納品先 | `1` | 受付が聞ける回数 |
| `question_deadline_weekdays` | 納品先 | `5` | 回答の期限（営業日） |
| `assumption_max_items` | 納品先 | `64` | 受付が自分で決めて記録してよい点の数 |
| `chain.retry_backoff_base_seconds` | 実行環境 | `60` | 待ちの最初の間隔 |
| `chain.retry_backoff_max_seconds` | 実行環境 | `1800` | 待ちの最長の間隔 |
| `chain.retry_max_attempts` | 実行環境 | `0`（上限なし） | 1 工程の配り直しの上限。**入れると終端が増える** |
| `chain.retry_notice_attempts` | 実行環境 | `3` | 何回目の試行で進行中を知らせるか |
| `chain.deliver.go_gate` | 実行環境 | `off` | `required` で本番の前に人の Go を待つ。**入れると人を待つ** |
| `chain.deliver.go_wait_seconds` | 実行環境 | 7 日 | Go を待つ上限 |
| `run_deadline_hours` | 実行環境 | `8` | 実行全体の期限 |
...
**外部チケットから本番反映までの制御流と、本番へ進める成果物を決める収束ゲート。**

1. **外部トラッカー起点** — GitHub イベント以外を入口にする（第 1 号は Backlog）
2. **成果物の生成と修正** — AI が候補を作り、敵対レビューの指摘を受けて次の revision を作る
3. **敵対レビュー** — 同一 revision を複数種類のフロンティア AI へ同時に渡し、進んでいるあいだ反復する。止まったら裁定する
...
本番への昇格は、一度のレビュー結果では決めない。**同一 revision への並列敵対レビュー、指摘に基づく修正、全判定のやり直しを繰り返す。** 繰り返しを止めるのは回数ではなく、収束したか、同じことを繰り返したか（そのときは本体が裁定する）である。実装の詳細と設定は [いまの契約](#いまの契約実装済みの動き)。
...
stateDiagram-v2
    ...
    Working --> Stage
    Stage --> Promoting: 収束
    Stage --> Revising: 非収束 かつ 進んでいる
    Revising --> Stage: 新しい revision
    Stage --> Arbitrating: 直前の巡と同じ
    Arbitrating --> Stage: 指摘を退けて、この巡を判定し直す
    Arbitrating --> Revising: 足りないと裁定して、次の巡へ
    Stage --> Failed: 判定不能 / 記録の天井 / 実行全体の期限
...
| 上限 | **改訂 (2026-09-26)** | **回数の上限は既定では置かない**（`max_rounds` 既定 0 = 無制限）。繰り返しを止めるのは収束と裁定であって数ではない。運用者が数を入れたときだけその数で止まる。記録の天井は 50 巡。**`max_stages` はもう巡を止めない** — 1〜5 の必須欄として残っているだけ |
| 停滞 | **実装済み** | 連続一致に加え、消えた同じ指摘や以前の不合格と同じ変更へ戻る往復を裁定する（`stagnation_repeat_rounds` 既定 1）。依頼者には聞かない |
| 記録の天井 | 合意済み | 50 巡に当たったら本番へ進めず、その時点の分類が持つコードでレポートして終了 |
...
「全判定者の指摘がゼロ」は最も単純な候補だが、採用は未決定。複数の非決定的な AI は、新しい指摘、重複、相反する提案、深刻度の揺れを出し続ける可能性があるため、実際に何巡で終わるかを検証して調整する必要がある。指摘が止まらない場合に依頼を落とさずに終端させる仕組みは、実装では停滞の裁定が受け持っている（[いまの契約](#いまの契約実装済みの動き)）。
...
**改訂 (2026-09-26)**: 当初は `max_stages` に暫定上限 3 を置き、到達したら人間へ報告して停止する設計だった。これは撤回した。**回数で打ち切ると、夜に投げた依頼が 3 巡目の朝に人待ちで止まる** — この体系がいちばん避けたい終わり方である。いまは回数ではなく「進んでいるか」で続け、進まなくなったら本体が裁定して先へ進める。参考にした測定（同一モデルの自己修復では利得の大半 76〜95% が最初の 2 ラウンドで飽和する。競合調査 (内部調査、非公開) §5.3, §6）は、**3 巡でだいたい収束する**という予測であって、**3 巡で打ち切ってよい**という根拠ではなかった。収束が早いなら上限は要らず、遅いなら上限は依頼を落とすだけになる。
...
**合意済み (2026-09-26)**: **人に聞かない保証の始点は、受付（着手可否の判定）を通過したときである。** その手前では質問が 1 巡だけ起こりうる。通過した後に人を待つのは、運用者が `chain.deliver.go_gate` を `required` にしたときと、`chain.retry_max_attempts` に値を入れたときの 2 つだけで、どちらも既定では起きない。詳細と既定値は [いまの契約](#いまの契約実装済みの動き)。

**設計提案**: 人間の判断を前段へ移し、自動本番反映を許す mode、対象 repo / environment、変更可能範囲、本番反映経路、停止条件を事前に承認する。

**設計提案**: 判定不能、scope 拡大要求、本番反映失敗、本番確認失敗は自動で別経路へ迂回せず、人間へ報告して停止する。
...
この公理自体と各欄の採用は要合意。現時点の候補では、判定者集合、終端の規則（収束・裁定・期限）、収束ポリシー、変更可能範囲、自動反映の対象外領域（例: スキーマ変更、認証・決済 — 国内で AI レビューのみの自動マージを運用する事例も、この種の高リスク領域は人間に残している。競合調査 §8）、成果物の同一性、本番反映経路、本番完了条件、可逆性（rollback 手順と stop owner が実在すること）、失敗時の停止と報告を定義する。収束ポリシーと本番反映契約が未設計なので、モード登録の契約も未完成である。
...
- **同じことを繰り返す巡を、繰り返したまま続けない。** 直前の巡と指摘・変更が完全一致したら本体が裁定し、依頼者に差し戻さずに先へ進める
- **各ステージの全判定者は同一 revision / artifact digest を評価する**
- **本番へ届かずに終わるときは、各ステージの指摘・修正・未解決事項を報告する**
- **どの終わり方でも、チケットに最終コメントを 1 件残す**
...
巡の上限・再試行・失敗処理の実装済みの契約は [いまの契約](#いまの契約実装済みの動き)。以下はモード契約の形についての設計提案である。

- 巡の数に既定の上限は置かない。止めるのは収束と裁定と記録の天井、そして実行全体の期限である。判定者数・時間・token・費用は上限を持つ
...
- **聞くのは受付だけで、受付を通ったら誰にも何も聞かない。** 保証の始点は受付の通過であり、その後に人を待つのは運用者が選んだ 2 つの例外だけ（[いまの契約](#いまの契約実装済みの動き)）。実装・レビュー・検証・納品の途中で仕様が曖昧だと分かっても、依頼者に差し戻さず本体が決めて記録する
- M1 の回答者はチケット作成者 1 名とする（トラッカーが記録している作成者を使う。別途 allowlist を持たない）
- 受付は、残っている疑問を 1 つのコメントにまとめて一度だけ聞く。件数は納品先の設定 `question_max_items`（既定 10、上限 19）で決まる
- 受付が聞く回数は `question_max_rounds`（既定 1）。この回数を使い切ったら、受付は残った点を自分で判断し、前提として記録して【実装方針】コメントに出す
- `questions` を `none` にした納品先では一切聞かず、すべて受付が判断する。`minimal`（既定）は擁護できる既定が立たない点だけを聞き、`normal` は従来どおり許可 dimension の曖昧さを聞いてよい
- 回答者は同じチケットへ `回答 C1` のように質問 revision を付けた新規コメントを投稿する。本文編集や bot 自身のコメントは回答にしない
- **質問コメントには、各選択肢の直下にそのまま投稿できる回答行（例: `回答 C1 Q1:a`）を併記し、さらに全問をまとめて答える回答テンプレート（`回答 C1` に続けて `Q1: _` を問い数だけ）を末尾に置く**（タイプさせない — 合意済み 2026-08-04。10 問を 1 コメントで返せることが条件）
...
- 要質問時は対象 repo の変更ゼロで、許可された dimension の質問を納品先が決めた件数まで 1 コメントで指定回答者へ通知する
- 同じチケット上の現在の質問 revision と回答者が一致する新規コメントだけを回答として採用する
- 回答から作った immutable input revision で 10 分以内を SLO として受領を通知し、clarification revision ごとに 1 回、`question_max_rounds`（既定 1）の回数だけ書き込み前の再開を許す
- その回数を超えて質問せず、残った点は受付が最も擁護できる既定を置いて進み、置いた前提を記録する
...
sequenceDiagram
    ...
    alt 繋いだ範囲外 / 重複
        ...
        loop 収束・裁定・失敗のいずれかまで（回数の既定上限なし）
            ...
            alt 収束
                ...
                Note over W: 本体が裁定する。指摘を退けたらこの巡を判定し直し、足りなければ次のrevisionへ（依頼者には聞かない）
            else 判定不能 / 記録の天井 / 実行全体の期限
                Note over W: 到達した状態と未解決findingを添えて終了
            end
        end
        alt 収束済み
...
- **停滞の検査**: 直前の巡と指摘・変更が完全一致する巡が、依頼者への質問ではなく本体の裁定になり、次の巡が始まること
- **終端の検査**: 記録の天井（50 巡）に当たった場合、本番に変更がなく、全履歴を伴って終わること
- **設定どおりの検査**: `max_rounds` に数を入れた納品先は、その数で終わること。入れていない納品先は、その数で終わらないこと
...
- **公理の検査**: 収束ポリシー、変更範囲、本番反映経路、完了条件を欠く場合に、**本番へ出さずに報告して止まること**（入口で依頼を拒否することではない）
...
2. **clarification チケット** — 許可 dimension の曖昧さを意図的に 1 点含めて起票し、質問（残った疑問を 1 コメントにまとめて一度だけ）→ 起票者の回答 → 新しい入力 revision での再開 → 本番確認まで進む。回答の選択が本番の結果に実際に反映されたことを確認する
...
| 敵対レビューと修正を反復する | 一度のレビュー結果ではなく、指摘を反映した次 revision を再評価して本番昇格を決めるため |
| 各ステージで同一 candidate を複数種類の AI へ並列投入する | 異なる成果物や相互に影響された判定を同じ合議として扱わないため |
| 巡の回数で打ち切らない（2026-09-26 改訂） | 回数で打ち切ると、進んでいる依頼が朝に人待ちで止まる。止めるのは収束と裁定と期限であって数ではない。回数の上限は運用者が選べる設定として残す |
| 同じ巡の繰り返しは本体が裁定する（2026-09-26） | 非決定的な判定を無制限に回さないための止め方を、依頼者への質問ではなく本体の判断にするため |
| 本番へ届かずに終わるときは報告する | 非収束を合格へ読み替えず、人間が未解決事項を追えるようにするため |
...
| 最大ステージ到達後に自動延長する | 延長の可否を実行時に判断する形は採らなかった。上限そのものを既定から外し、止め方を収束・裁定・期限に置き換えた（2026-09-26） |
| `max_stages`（既定 3）で巡を打ち切る | 3 巡で収束するという測定を、3 巡で打ち切ってよい根拠に読み替えていた。収束が早いなら上限は要らず、遅いなら依頼を落とすだけになる（2026-09-26 撤回。欄は既存設定の互換のため残す） |
...
- **誤収束と非収束の両方が起こりうる。** 複数 AI が見逃した欠陥はそのまま通り、問題のない成果物でも指摘が止まらない可能性がある。後者は停滞の裁定が受け持つが、裁定そのものもモデルの判断である
- **敵対レビューの結果は再現しない。** 同じ candidate でも実行ごとに finding が変わり、model 更新で過去の判断も再現しなくなる
- **コストは無視できない。** 1 巡ごとに複数 AI を並列実行し、修正後は全判定をやり直す。**巡の回数に既定の上限が無い以上、費用の上限は鍵の側（提供元の予算・利用上限）に置く。** 本体は予算を持たない
...
一回のレビュー実行方法は本 repo の独立監査から知見を得られたが、レビュー → 修正 → 再レビューを収束させる規則はまだ実証していない。ここを自然言語の「いい感じに合意したら」にせず、状態機械と構造化データへ落とすことが最優先である。いまの実装は「進んでいるあいだ続け、完全一致の繰り返しは裁定する」という規則になっている（[いまの契約](#いまの契約実装済みの動き)）。
...
経緯を渡さない、同一 candidate を並列に渡す、観点を割る、「指摘なし」も明記させる、根拠を必須にする、判定者を相互に見せない、という条件はパイプライン外のレビューにも利用できる。ただし、本体の主目的はその知見単体ではなく、**チケットから本番反映までを人手を挟まずに終わらせること**である。
...
| 巡 / stage | 敵対レビューと修正の 1 回分。回数に既定の上限は無く、止めるのは収束・裁定・記録の天井、そして実行全体の期限（[いまの契約](#いまの契約実装済みの動き)） |
| `max_stages` | かつての巡の上限。いまは巡を止めない 1〜5 の必須欄（[いまの契約](#いまの契約実装済みの動き)） |
| 裁定 | 直前の巡と同じことを繰り返した依頼について、本体が受入条件を読んで指摘の要否を決めること。指摘を退けたらその巡を判定し直し、足りないと裁定したら次の巡が始まる |
...
| 2026-09-26 | **巡の上限の契約を実装に合わせて改訂。** `max_stages`（既定 3）で打ち切る契約を撤回し、「進んでいるあいだ続け、直前の巡と完全一致したら本体が裁定する」（`max_rounds` 既定 0、`stagnation_repeat_rounds` 既定 1、記録の天井 50 巡）に書き換えた。保証の始点を受付（readiness）通過と明記し、`go_gate: required` と `retry_max_attempts` を「運用者が選んだ例外」として契約に載せた。`internal_failed` で終わる名前付きの条件を列挙し、受付の記録が読めなくなった場合は受付をやり直す扱いに変えた。staging 止まりの最終コメントが「後で自動反映する」と「人が反映する」を同時に述べていたのを、実際に起きること 1 つへ直した。[いまの契約](#いまの契約実装済みの動き)を新設 |
