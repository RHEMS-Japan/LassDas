...
| `triggers` | `on:` に書いてよいイベント。`push` と `pull_request` は **branches でリリース用・取り込み用のブランチに絞ったときだけ**使える。`workflow_dispatch` はそのまま使える |
...
3. `${{ }}` の中は文字列検索ではなく**式として**読む。`toJSON(secrets)` や `secrets[format(...)]` のような書き方は、許可した名前かどうかに関わらず通らない
4. 本体が workflow を書いた実行では、**反映を記録するコミットの方針 (`github_contract.staging_digest_commit`) を本体が自分で書き込む**。その形を作るのは本体が書いた workflow なので、観測して決めるものではなくなるため。書き込みは Pull Request ができた後に 1 回だけ行われ、走行中の実行はやり直しになりません
