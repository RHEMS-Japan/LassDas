...
| A moment that passes ending a whole round on its first occurrence: the gateway's own 500, its 408, a provider's 529, and a connection the network dropped were each a permanent failure, while its 502 and 503 were asked again. An investigating round spends up to sixty calls, so one of them was enough | `internal/worker` `TestAMomentThatPassesIsAskedAgainHoweverItArrived`, `TestADroppedConnectionIsAskedAgain`, `TestASpentAllowanceIsNotAskedAgainByTheTransport` | audit, 2026-09-09 |
