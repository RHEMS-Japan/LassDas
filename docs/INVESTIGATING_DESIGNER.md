...
- モデルは結果の**抜粋**を読む (1 件あたり 32 KiB まで)。全文は記録に残る。**抜粋で切れた出力は、`{"read":{"id":"m-0001","offset":<byte>}}` で続きを 32 KiB ずつ読める** (関所が記録から切り出す。probe は走らず記録も増えない。回数は別枠で上限 120)。件数や有無を結論に使う出力は、読み切ってから書く。読めなかったときだけ「抜粋しか見ていない」と記録に書く (2026-09-07: イベント一覧 94 KB を抜粋だけで数えた報告が、全文を読む根拠レビューに 3 巡差し戻され不成立になった)
