...
| A reception failure the runner had no words for, ending the ticket with the failure class and nothing else: the cause lived only in the pod log, so the requester could not tell whether to send the same ticket again | `internal/runner` `TestAReceptionStageThatNeverGotAnAnswerTellsTheRequesterSo`, `TestTheTransportsOwnFailureAlsoReachesTheRequester`, `TestAnUnnamedReceptionFailureStillLeavesANote`, `TestAnUnexplainedReceptionFailureInventsNoCause`, `TestATicketCannotChooseTheNoteItsRequesterIsShown` | live, 2026-09-09 |
