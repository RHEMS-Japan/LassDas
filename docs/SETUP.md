...
**いまの段階では、この答えは受付の判断を動かさない。**受付は今までどおり自分で決めて、依頼者に聞くか聞かないかも今までどおり決める。この役は、その判断と並べて記録を取り、「聞く / 聞かない」をこの答えで変えてよいかを測るために置いてある。動きを変えるのは次の段階。
...
- **1 回の判定は速くて安い。**実際の依頼 67 件で測ったところ、返るまで中央値 0.19 秒 (最大 0.36 秒)、1 件あたり 0.000076 ドル。料金は入力の分だけで、出力は無料
