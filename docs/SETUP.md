...
| `proceed_threshold` | 省略可。0.5 以上 1.0 以下。範囲の外を書くと設定の読み込みで止まる。**省略すると 1.0** (下の「既定を 1.0 にした理由」) |
...
**この版では、新しい判定基準での実測が取れていない。**判定の窓口に通す鍵がこの作業では使えず、生きた呼び出しを 1 回も通せなかった。数字を測らずに「0.85 くらいなら安全」と書くのは、測った顔をした当て推量になる。

そこで既定は 1.0 — **判定が完全に確信したときしか何も起きない値** — にしてある。実測を取った人が、下の測り方で数字を出してから下げる。

参考にできる実測は 1 つだけあり、**これは前の判定基準で測ったもの**なので、そのまま閾値の根拠にはできない。2026-09-26 に依頼 67 件を 1 回通したとき、「進めてよい」と返った 5 件の確信度は 0.69〜0.83 に収まり、1.00 は 1 件も無かった。**新しい基準文は「進める側」を選びやすく書き直してあるので、この分布は動く。**
...
- 出力の最後に、閾値ごとの表が出る。**「受付が進んだ依頼のうち何件が yes になるか」と「受付が質問した依頼のうち何件が yes になってしまうか」の 2 列**。後者が 0 のままで前者が一番大きい値が、置いてよい上限
...
- **1 回の判定は速くて安い。**料金は入力の分だけで、出力は無料。**下の数字は 2026-09-26 に、実際の依頼 67 件を 1 回通して測った 1 回分の実測で、日や依頼の内容が変われば動く。**返るまで中央値 0.19 秒 (最大 0.36 秒)、1 件あたり 0.000076 ドル
