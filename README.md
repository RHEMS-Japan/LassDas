...
> **未反映。** この期限は別の変更で入る。現在の main には無く、いまは記録の天井 50 巡と、はしごの待ちが実質の上限になっている。
...
| `run_deadline_hours` | 実行環境 | `8`（**未反映**） | 実行全体の期限 |
...
stateDiagram-v2
    ...
    Stage --> Failed: 判定不能 / 記録の天井 / 実行全体の期限（未反映）
...
- 巡の数に既定の上限は置かない。止めるのは収束と裁定と記録の天井、そして実行全体の期限（**未反映**）である。判定者数・時間・token・費用は上限を持つ
...
| 巡 / stage | 敵対レビューと修正の 1 回分。回数に既定の上限は無く、止めるのは収束・裁定・記録の天井、そして実行全体の期限（**未反映**、[いまの契約](#いまの契約実装済みの動き)） |
