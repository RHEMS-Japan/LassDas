...
- モデルは結果の**抜粋**を読む (1 件あたり 32 KiB まで)。全文は記録に残る。**抜粋で切れた出力は、`{"read":{"id":"m-0001","offset":<byte>}}` で続きを 32 KiB ずつ読める** (関所が記録から切り出す。抜粋も窓も文字の途中では切らない。probe は走らず記録も増えない。回数は別枠で 1 巡あたり上限 120 (拒否も数える)。表示した窓の数は `invocation.json` に `reads` として残る)。件数や有無を結論に使う出力は、読み切ってから書く。読めなかったときだけ「抜粋しか見ていない」と記録に書く (2026-09-07: イベント一覧 94 KB を抜粋だけで数えた報告が、全文を読む根拠レビューに 3 巡差し戻され不成立になった) revise 巡では `earlier_records` に、その依頼でこれまでに走った記録の一覧 (id・probe・引数・exit_code・出力バイト数・保存済みバイト数・拒否か) が入る。offset 0 から読み直せる (`stored_bytes` が 0 の記録と拒否された記録には読むものが無い)。一覧を作れなかったときは `earlier_records_unavailable: true` が立つ。
