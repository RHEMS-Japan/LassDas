...
- **迷ったら止まる** の形: 写し役は **自分の作業コピーの根 (target-repo 直下)** に `revise-design.json` (理由 1〜600 バイト・どの項目か) を書いて **exit 0** で終わる (Hermes の契約では exit 0 = エージェント完了)。写し役は別 UID なので run dir には書けず、書ける場所はここだけ (#103)。写し役のカード (`run-instruction --role applier`) がこのファイルを見つけたら、木の走査 (範囲検査) をせずに異議として読み、設計巡の `objection.json` として確定記録にし、ファイルを巡のディレクトリへ移して木から消し、**そのカードを非ゼロで終える**。他の編集と並んだ異議・空や 600 バイト超の理由は名指しで拒否し、ファイルは消す (次の巡の走査が拾わないように)。従来の検知点 (review-a カード冒頭の `seal-candidate` が run dir 直下の同名ファイルを読む経路) も残る。進行係は終了コードを見られない (盤面のカードは状態と block_kind しか持たず、既存の分類も記録の有無で行う) ので、**run dir の `design-objection.json` の有無で「設計に戻す」と分類する** (失敗ではない)。その巡の実装・レビューのカードを退役させ、`design-objection.json` と `revise-design.json` を `history/stage-N/` に移して stage-N を閉じる (`decision.json` は書かない。`currentRound()` は `decision.json` の有無で巡を数えるので、巡は進まない。次に封緘するカードが同じファイルを再検出しない)、設計の次の巡 (`investigate` カード d<N+1>) を作る。実装の巡番号は進めない。異議は設計の巡として `design_max_rounds` を消費する
