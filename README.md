...
- **3 回聞いても読める答えが返らなかったら、本体が自分で読む。** 依頼は**チケットの本文そのまま**、読み取った項目は無し、種別は「変更」。**質問は 1 問も出さない**
- **依頼は実装役へ進む。** 何を変えるかは実装役がリポジトリを読んで決める。受付の読み取り役が本来補うはずだったものは、そのリポジトリの中にある
- **起票者には 1 行で伝わる。** 【実装方針】コメントと最終コメントの前提の欄に「受付の読み取り役が読める形で答えなかったため、依頼はチケットの本文をそのまま実装役へ渡し、受付からの質問はしていません」が出る。両方に出て、依頼 1 件につき 1 回だけ出る
