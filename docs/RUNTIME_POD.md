...
| A model call that spends its whole allowance, leaving no answer, no usage and no cost, and ending the delivery where it was asked; and a remedy for it that cannot fire, because the transport collapses its cause into a message | `internal/worker` `TestASpentAllowanceJoinsTheProvidersRetryLadder`, `TestTheAllowanceRetryFiresOnTheRealTransportsError` | live, 2026-09-09 (the reception, after thirteen runs) |
