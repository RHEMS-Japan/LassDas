...
func terminalCommentContent(report TerminalReportRequest, reportDigest string, deliveryContinues bool) string {
	message := map[TerminalCode]string{
		...
		TerminalDesignRoundsSpent:              "直し方の設計は合意できましたが、その通りに書いた変更のレビューが「設計そのものを変えるべき」と判断しました。設計をやり直せる回数を使い切っていたため、書いた変更は取り下げ、リポジトリは変更せず停止しました。争点は運用担当者が確認し、必要に応じてこのチケットでお知らせします。",
	}[report.Code]
	if message == "" {
