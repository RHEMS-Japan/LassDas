...
| An applier's objection written where its instruction says (the root of its working copy) ending the run as a change outside the writable scope instead of reopening the design; an objection beside other edits, or with an empty or overlong reason, accepted or left in the tree; an objection left behind by a run that died, read by the card's second attempt as this round's | `cmd/worker` `TestRunInstructionSealsTheAppliersObjectionWrittenInTheWorkingDirectory`, `TestAFailedApplierLeavesNoObjectionForTheNextAttempt`; `internal/attendant` `TestApplyCardObjectionReopensDesignRound`; `internal/runner` `TestChainApplyCardCarriesTheDesignAndTheObjectionDestination`, `TestApplyInstructionCarriesThePreviousRoundsFindings` (rule pins) | live, 2026-09-09 (#103) |
