...
|---|---|---|---|
| 本番の前に人の Go を待つ | `chain.deliver.go_gate` | `off`（待たない） | `required` にすると、staging の確認が通ったあと、チケットに「Go」と書かれるまで本番へ進まない。待つ上限は `chain.deliver.go_wait_seconds`（既定 7 日）で、過ぎたら本番未反映のまま正直に報告して終わる |
