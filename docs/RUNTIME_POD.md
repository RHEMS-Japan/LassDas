...
The status board lists the runs; each row links to `/tickets/<issue key>`, one page per ticket that shows everything its run recorded: the reception (what the intake read, the readiness decision and its design line, the assumptions and questions), each implementation round with the changed files and the implementer's report, each reviewer's verdict with its findings — or, when a reviewer returned no verdict, the tail of its transcript — the sandbox validation, the pull request, the merge, the delivery verdict per phase, the recorded costs of every priced model call (review calls record no price, and the page says so), and the ending with the failed step and the failure class in the requester's words. `/api/tickets/<key>` serves that view together with the board row; `/api/tickets/<key>/records/<name>` serves one raw record, by page name only (never a path), from the run directory the board row names. Everything shown passes the same secret scan as a probe output: masked values keep their markers, a record the scan refuses whole is not shown. The run directories are read from `LASSDAS_RUNS_ROOT`, or the `runs` directory beside the status directory (the pod's `/data` layout). The page changes nothing: it has no action of its own, and the board's credentials guard it.
...
| A run whose failure could be read only in the pod — a reviewer that returned no verdict, a key whose allowance ran out, a design decision, the price of the calls — with the board showing one line and the ticket a paragraph; the ticket page now shows the whole run from its own records, masked the way probe outputs are, by the key the board lists and never by a path | `internal/ticketview` `TestBuildAssemblesTheRunInOrderWithCostsAndMaskedSecrets`, `TestRecordPathServesOnlyKnownNames`, `TestShownRefusesWholeSecretsAndBoundsLength`; `cmd/statusboard` `TestTicketAPIResolvesTheKeyThroughTheBoardOnly`, `TestTicketRecordsAreAllowListedAndMasked`, `TestTicketPageIsServedForKeysOnly`, `TestBoardHTMLLinksEachCardToItsTicketPage` | live, 2026-09-15 (two failed deliveries whose causes were found only through the pod's records and the gateway's database) |
