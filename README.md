...
- **同じことを繰り返したら、本体が裁定する。** 直前の巡と、指摘（座席・指摘コード・対象パス）が完全に同じか、書いたバイト列が同じパスに対して完全に同じなら、停滞とみなす。何巡続いたら裁定するかは `stagnation_repeat_rounds`（既定 1 = 直前の巡と同じなら、その場で）。裁定役はチケットの受入条件を読み、「レビューが依頼より多くを求めている」なら指摘を退け、「変更が依頼に足りていない」なら実装役に何を満たすかを指示する。どちらでも次の巡が始まり、依頼者には何も聞かない
...
- 巡の数に既定の上限は置かない。止めるのは収束と裁定、そして実行全体の期限である。判定者数・時間・token・費用は上限を持つ
    ...
    alt 繋いだ範囲外 / 重複
        ...
        loop 収束・裁定・失敗のいずれかまで（回数の既定上限なし）
            ...
            alt 収束
                ...
                Note over W: 本体が裁定し、次のrevisionへ（依頼者には聞かない）
...
| 巡 / stage | 敵対レビューと修正の 1 回分。回数に既定の上限は無く、止めるのは収束・裁定・記録の天井・実行全体の期限 |
| `max_stages` | かつての巡の上限。いまは巡を止めない 1〜5 の必須欄（[いまの契約](#いまの契約実装済みの動き)） |
| 裁定 | 直前の巡と同じことを繰り返した依頼について、本体が受入条件を読んで指摘の要否を決め、次の巡を始めること |
