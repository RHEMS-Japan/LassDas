...
1.5. **利用者が、モデルの鍵に利用上限を付ける。**本体は自分では費用を打ち切らない — 依頼が終わるまで手を替えて進み続けるので、止まるのは提供元が鍵を断ったときだけ。上限の無い鍵を渡すと、誰も気づかないまま使い切る経路がそこだけ残る。
   ...
   - 上限に達したレビュー役は、設定済みの候補のうち、独自の起動設定を持ち、他の判定役と提供元が重ならないものを先に試す。候補の鍵にもそれぞれ上限を付ける。本体は上限の変更や新しい鍵の探索をしない
   - 使える候補が無い・候補も拒まれた場合は、課題へ状況を知らせて待つ。再試行が通れば再開するが、現行の実行期限による失敗終了は残る。待つことを納品済みとは扱わない
...
## 1 件にかける時間の上限 (`run_deadline_hours`)

**本体が動く場所の設定 (`runtime.json` の `chain`) に置く。既定は 8 時間で、書かなければそれが効く。**

本体は失敗しても止まらず、モデルと提供元を替え、経路を替え、道具を入れ直し、最後は間隔を空けて試し続ける。**直る失敗にはこれで十分だが、直らない失敗には終わりが無い** — 利用枠を誰も上げない、提供元が落ちたまま、AI が毎回「実現できません」と返してくる。そうなると依頼は朝になっても終わらず、課題には「まだ進めています」という通知が 1 件あるだけになる。

上限に達すると、本体は打ち切って**そこまでの結果を課題にコメントして終わる**。終わり方の名前は `deadline_reached` で、**内部エラー (`internal_failed`) とは別**。壊れたのではなく、時間を使い切っただけなので、盤面にも「処理時間の上限に達して終了」と出る。

| 書き方 | 効き方 |
|---|---|
| 書かない | 8 時間 |
| `"run_deadline_hours": 20` | 20 時間 |
| `"run_deadline_hours": 0` | **起動しない。**上限の無い依頼は終われないので、読み込みの時点で断る |

**時計は依頼を掴んだ時点から。**受付の質問に答えてもらうのを待っている間は数えない (質問が出ると依頼はいったん待ち行列に戻り、答えが入って掴み直した時点から数え直す)。

**上限で打ち切られた依頼は自動で再開しない。**必要なものを用意してから、同じ内容で出し直す。
