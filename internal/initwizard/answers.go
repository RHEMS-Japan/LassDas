...
// fromDistribution is the help for the four answers the distributor's
// note carries; each line of a check is read on its own.
const fromDistribution = "配布者の案内 (~/" + DistributionFile + ") から自動で埋まる。無ければ `lassdas setup install` が未実行なので、配布者の案内をもらって install する"
