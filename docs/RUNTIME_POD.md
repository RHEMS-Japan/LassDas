...
| A sound review discarded over its label — one non-ASCII word in a finding code failed the whole card and ended the delivery as a model failure | `internal/worker/investigate` `TestAFindingLabelIsMadeToFitInsteadOfEndingTheReview`; `internal/worker` `TestTheAgentsFindingLabelIsNormalisedAsItIsRead` | live, 2026-09-09 |
