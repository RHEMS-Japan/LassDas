...
func TestModesWriteInitialAndPeriodicSnapshots(t *testing.T) {
	for _, mode := range []string{"runner", "cards"} {
		for _, chainInterval := range []string{"0", "20ms"} {
			for _, once := range []bool{true, false} {
				t.Run(fmt.Sprintf("%s/chain=%s/once=%v", mode, chainInterval, once), func(t *testing.T) {
					...
					for {
						...
						case <-done:
							...
							if readSnapshotTime(boardPath).IsZero() {
								t.Fatal("mode produced no initial snapshot")
							}
							return
						case <-ticker.C:
						...
						case <-deadline.C:
							t.Fatal("mode did not publish progress")
						}
					}
				})
			}
...
func TestChainPollingIsCardsOnlyAndIndependentOfObservation(t *testing.T) {
	for _, mode := range []string{"runner", "cards"} {
		for _, chainInterval := range []string{"0", "20ms"} {
			t.Run(mode+"/chain="+chainInterval, func(t *testing.T) {
				configPath, boardPath, callsPath := observationFixture(t, mode)
				...
				if mode == "cards" && chainInterval != "0" {
					if calls() < 5 {
						t.Fatal("cards did not advance independently")
					}
				} else if calls() != 2 {
					t.Fatalf("unexpected fast sync in %s with chain=%s: %d reads", mode, chainInterval, calls())
				}
...
func observationFixture(t *testing.T, mode string) (configPath, boardPath, callsPath string) {
	t.Helper()
	root := t.TempDir()
	...
	config := map[string]any{
		"orchestration": mode, "ledger_path": ledgerPath,
